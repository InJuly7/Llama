...
def test_llama_rope():
    ...
    position_ids = torch.arange(config.max_position_embeddings * 2, dtype=torch.long, device=config.device).unsqueeze(0)  # [B, S]
    ...
    cos, sin = rope.forward(q, position_ids)  # [B, max_position_embeddings * 2, D]
    # 在 N 维度扩展
